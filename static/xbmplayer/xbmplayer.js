...
/***/ (function(module, exports, __webpack_require__) {
	...
	root.xbmPlayer = function (options) {
	  // 检查必填选项
	  ...
	  var musicConf = options.music,
	      target = selector.$(options.target) || document.querySelector('.xbmplayer'),
	      ...
	      currentThemeClass = theme === _constants.THEME_DEFAULT ? 'xbmplayer-container' : 'xbmplayer-container-mini',
	      containerClass = currentThemeClass + ' ' + (hasLrc ? 'xbmplayer-haslrc' : '') + ' xbmplayer-isloading',
	      playerHTMLContent = '<div class="' + containerClass + '">\n                             <audio src=' + musicConf.src + ' preload="auto"></audio>\n                             <div class="xbmplayer-info">\n                             <div class="xbmplayer-info-cover"><img src=' + coverSrc + ' alt="cd-cover"></div>\n                             <div class="xbmplayer-meta">\n                             <div class="xbmplayer-meta-title">' + musicConf.title + '</div>\n                             <div class="xbmplayer-meta-author">' + musicConf.author + '</div>\n                             <div class="xbmplayer-meta-time-tick"><span class="xbmplayer-meta-time-tick-text"></span></div>\n                             </div>\n                             </div>\n                             <canvas class="xbmplayer-spectrum"></canvas>\n                             <div class="xbmplayer-lyric"><div class="xbmplayer-lyric-area"></div></div>\n                             <div class="xbmplayer-control"><div class="xbmplayer-control-play"><i class="xbicon-play"></i><i class="xbicon-pause"></i></div></div>\n                             <div class="xbmplayer-volume-bg"><div class="xbmplayer-volume"><i class="xbicon-volume"></i><div class="xbmplayer-volume-progress"></div></div></div>\n                             <div class="xbmplayer-duration"><i class="xbicon-clock"></i><span class="xbmplayer-duration-text">loading</span></div>\n                             <div class="xbmplayer-loadingsign"><i class="xbicon-spin animate-spin"></i>loading</div>\n                             <div class="xbmplayer-timeline-bg"><div class="xbmplayer-timeline"><div class="xbmplayer-timeline-passed"></div></div></div>\n                             </div>';

	  target.innerHTML = playerHTMLContent;

	  var xbmplayerContainer = target.querySelector('.' + currentThemeClass),
	      _selector$init$select = selector.init(xbmplayerContainer).select(['audio', '.xbmplayer-control-play', '.xbmplayer-meta-time-tick-text', '.xbmplayer-duration', '.xbmplayer-timeline', '.xbmplayer-timeline-passed', '.xbmplayer-volume', '.xbmplayer-volume-progress', '.xbmplayer-lyric-area', '.xbmplayer-spectrum']),
	  ...
	  function handleAudioEnd() {
	    if (loop) {
	      ...
	      utils.removeClass(xbmplayerContainer, 'xbmplayer-isplaying');
	    }
	  }
	  ...
	  function handleCanPlayThrough() {
	    ...
	    setTimeout(function () {
	      utils.removeClass(xbmplayerContainer, 'xbmplayer-isloading');
	      timeCount.querySelector('.xbmplayer-duration-text').innerText = utils.parseSec(duration.toFixed(0));
	    }, 1000);
	  }
	  ...
	  function handleTimeUpdate() {
	    ...
	    if (hasLrc && theme === _constants.THEME_DEFAULT) {
	      ...
	      if (!tempLrcLine.className.includes('xbmplayer-lyric-current')) {
	        utils.removeClass(lyricArea.querySelector('.xbmplayer-lyric-current'), 'xbmplayer-lyric-current');
	        if (lyricArea.querySelector('.xbmplayer-lyric-pre')) {
	          utils.removeClass(lyricArea.querySelector('.xbmplayer-lyric-pre'), 'xbmplayer-lyric-pre');
	        }
	        if (lyricArea.querySelector('.xbmplayer-lyric-next')) {
	          utils.removeClass(lyricArea.querySelector('.xbmplayer-lyric-next'), 'xbmplayer-lyric-next');
	        }
	        utils.addClass(tempLrcLine, 'xbmplayer-lyric-current');
	        if (tempLrcLinePre) {
	          utils.addClass(tempLrcLinePre, 'xbmplayer-lyric-pre');
	        }
	        if (tempLrcLineNext) {
	          utils.addClass(tempLrcLineNext, 'xbmplayer-lyric-next');
	        }
	  ...
	  function handlePlayClick() {
	    ...
	    if (audio.paused) {
	      ...
	      // 播放状态中可以用滑轮调节音量
	      xbmplayerContainer.addEventListener('mousewheel', function handleMouseWheel() {
	        var timer = null;
	        var step = 0.05;
	        _handleMouseWheel = function _handleMouseWheel(event) {
	          ...
	          if (!xbmplayerContainer.className.includes('xbmplayer-adjusting-volume')) {
	            utils.addClass(xbmplayerContainer, 'xbmplayer-adjusting-volume');
	          }
	          ...
	          timer = setTimeout(function () {
	            utils.removeClass(xbmplayerContainer, 'xbmplayer-adjusting-volume');
	          }, 1000);
	        };
	      ...
	      audio.pause();
	      spectrum.stop();
	      xbmplayerContainer.removeEventListener('mousewheel', _handleMouseWheel);
	    }
	    utils.toggleClass(xbmplayerContainer, 'xbmplayer-isplaying');
	  }
	  ...
	  function play() {
	    if (audio.paused) {
	      utils.addClass(xbmplayerContainer, 'xbmplayer-isplaying');
	      audio.play();
	    }
	  }
	  ...
	  function toggleTheme() {
	    ...
	    utils.addClass(xbmplayerContainer, 'xbmplayer-changing-theme');
	    ...
	    function loop() {
	      count++;
	      xbmplayerContainer.style.opacity -= step;
	      if (xbmplayerContainer.style.opacity <= 0) {
	        step *= -1;
	        xbmplayerContainer.style.opacity = 0;
	        utils.toggleClass(xbmplayerContainer, 'xbmplayer-container-mini');
	        utils.toggleClass(xbmplayerContainer, 'xbmplayer-container');
	      }
	      if (xbmplayerContainer.style.opacity < 1 && count < maxCount) {
	        requestAnimationFrame(loop);
	      } else {
	        setTimeout(function () {
	          utils.removeClass(xbmplayerContainer, 'xbmplayer-changing-theme');
	        }, 500);
	      }
	    }
	  }
...
/***/ (function(module, exports) {
	...
	var THEME_MINI = 'mini';
	var LYRIC_CURRENT_CLASS = 'xbmplayer-lyric-current';
	var LYRIC_NEXT_CLASS = 'xbmplayer-lyric-next';
